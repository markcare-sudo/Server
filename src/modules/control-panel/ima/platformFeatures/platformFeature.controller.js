...
async function getOne(req, res, next) {
  try {
    ...
    return res.json({ success: true, message: "Feature fletched successfully", data });
  } catch (e) {
    next(e);
  }
}
...
async function createBulk(req, res, next) {
  try {
    ...
    return res.status(201).json({ success: true, message: "Bulk Feaures created successfully", data });
  } catch (e) {
    next(e);
  }
}
